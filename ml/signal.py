...
def score_latest(features: pd.DataFrame, asset: AssetConfig, repo_root: Path | None = None) -> dict:
    root = repo_root or Path(__file__).resolve().parents[1]
    model_path = root / "ml" / "models" / asset.id / "svm.joblib"
    as_of = pd.Timestamp(features.iloc[-1]["open_time"]).isoformat().replace("+00:00", "Z") if len(features) else None
    ...
    probability_up = float(model.predict_proba(latest)[0][1]) if hasattr(model, "predict_proba") else None
